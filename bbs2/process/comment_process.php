...
{       /* 댓글을 안썻거나 게시판 번호가 안넘어 오면 표시 */
    ...
    {
         $query = "INSERT INTO `bbs2_comment` (`no`, `bbs_no`, `user_id`, `content`, `date`) VALUES
        (NULL, '$bbs_no', '$user_id', '$content', current_timestamp())";
        $result = mysqli_query($conn, $query);
        if($result)
        {
            header ("Location: ../bbs2_view.php?no=$bbs_no");
        }
        else 
        {
...
else
{
    ...
    {
        $query = "UPDATE bbs2_comment SET content = '$content', mod_date = current_timestamp() where no = $comment_no ";
        $result = mysqli_query($conn, $query);
        if($result)
        {
            header ("Location: ../bbs2_view.php?no=$bbs_no");
