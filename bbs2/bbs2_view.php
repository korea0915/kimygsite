...
include "../header.php";
/* 게시판 번호로 테이블에서 값 조회 */
$query = "select * from bbs2 where no = $bbs_no";
...
?>
    ...
    <thead>
        <tr>
            ...
            <div class="d-flex flex-row-reverse bd-highlight"><a class='btn btn-secondary' href="./bbs2.php">목록</a></div>
        </tr>
    </thead>
    <tbody>
        <tr>
        ...
        <tr>
            ...
            <?php  
                  ...
                  //이미지 업로드했을시 이미지 표시  사이즈 조정 필요
                   if(isset($check['imgname']))
                   {
                    /* db에 저장된 이름 */
                    $img = $check['imgname'];
                    /* db에 저장되기 전 실제 이름 */
                    $imgname=$check['Rimgname'];
                    echo "<img src='./img/$img' alt='$imgname'>";
                   }
                   else{} 
            ?>
            </td>
...
<div class="card mb-5">
    ...
    $comment_query = "select * from bbs2_comment where bbs_no=$bbs_no";
